"""
12mo / 6mo bake-off: do AdxMa / EmaPsarTrend actually beat the live USD/CHF
(BreakoutAdx) and EUR/CHF (ZScoreAdx) profiles, or did they just win one
trendy week? Same mechanics as backtest_pair_sweep.py (H4, R:R 1.5/1.5, 2%).
"""
import inspect
import pandas as pd
...
for sym, engines in CANDIDATES.items():
    ...
    atr = _atr(df)
    print(f"\n### {sym}   ({len(df)} H4 bars, {df.index[0].date()} -> {df.index[-1].date()})")
