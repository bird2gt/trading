...
def _extract_bias(sections: list[str]) -> dict[str, tuple[int, str]]:
    ...
    бычий/медвежий → (±1, PARAMETERS) soft. Нейтральный → skipped (символ не
    пишется → бот торгует штатно по своей стратегии; auto-LOCK не ставим —
    он лишь гарантированный простой без доказанной пользы). Auto-LOCK остаётся
    ручным инструментом (LOCK в forecast-файле без origin). Unknown symbols skipped."""
...
def _write_bias_file(today: date, biases: dict[str, tuple[int, str]]) -> None:
    ...
    (soft, shrinks opposed trades); neutral view → symbol omitted (bot trades it
    normally). A manual forecast file (no `origin`) overrides this one."""
    if not biases:
    ...
    lines += [
        ...
        "PARAMETERS (встречные сделки ×0.5, не блок); нейтральный → символ не",
        "пишется (бот торгует штатно). Действует 1 день. Ручной forecast-файл",
        "(без origin) перекрывает этот авто-bias.",
        "",
    ]
